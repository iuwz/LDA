...
def create_app() -> FastAPI:
    ...
    async def read_current_user(current_user: UserInDB = Depends(get_current_user)):
        """
        Returns the current authenticated user's basic info including first and last name.
        """
        return {
    ...
    async def analyze_document_risk(input_data: RiskAnalysisInput, request: Request):
        ...
        return {"analysis_result": await analyze_risk(input_data.document_text, user_id, db)}
